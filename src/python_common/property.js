define([
    ...
    PythonCommon.prototype.optionValidation = function() {
        var required = [ 'vp_pyGetter', 'vp_pySetter' ];
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" placeholder="{2}"/>', 'vp_pyGetter', 'vp-input', 'function');
        tblLayoutRequire.addReqRow("Getter", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" placeholder="{2}"/>', 'vp_pySetter', 'vp-input', 'function');
        tblLayoutRequire.addReqRow("Setter", sbTagString.toString());
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        var getter = $(this.wrapSelector('#vp_pyGetter')).val();
        var setter = $(this.wrapSelector('#vp_pySetter')).val();
        code.appendFormat("property({0}, {1})", getter, setter);
